...
``` r
brfss_df_new |>
  ggplot(aes(y = mean_data_value, x = year, color = state)) +
  geom_line(alpha = 0.7) +
...
brfss_df |>
  ...
  ggplot(aes(y = data_value, x = response, color = response)) +
  geom_boxplot() +
  facet_wrap(~year) +
  labs(x = "Reponses", y = "Data Value",
       title = "2006 and 2010 Data Value vs Response")
```

<img src="p8105_hw3_yy3295_files/figure-gfm/unnamed-chunk-10-1.png" width="90%" />

\#Problem 3 Load the data, Tidy, merge and organize the data sets
...
nhanes_merge$bmi <- as.integer(nhanes_merge$bmi)
```
